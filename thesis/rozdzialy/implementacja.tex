...
Kod aplikacji składa się z wielu folderów, gdzie struktura i zawartość większości z nich została automatycznie wygnerowana przy tworzeniu projektu za pomocą zestawu narzędzi \textit{Flutter}, zapewniając elementy niezbędne do uruchomienia aplikacji na różnych platformach. Zostały one pokazane na rysunku \ref{drzewko_glowne}. Foldery \textit{android/}, \textit{ios/}, \textit{macos/}, \textit{windows/} oraz \textit{linux/} zawierają natywne projekty dla platform mobilnych oraz systemów operacyjnych. W katalogu \textit{.idea/} umieszczone są pliki konfiguracji dla \textit{Android Studio}, natomiast zawartość folderu \textit{build/} jest automatycznie generowana przy budowie aplikacji. Główny kod źródłowy, który został napisany, znajduje się w folderze \textit{lib/}.
...
\caption{Struktura plików projektu - główne foldery (nazwy katalogów ukrytych, takich jak \texttt{.idea/}, zapisano bez kropki ze względu na ograniczenia pakietu \texttt{dirtree})} 
...
\section{Generator list dźwięków}
W celu automatyzacji pracy i zmniejszenia ilości ręcznych czynności związanych z tworzeniem list dźwięków dla treningów oddechowych, stworzony został prosty generator list dźwięków. Narzędzie to umożliwia szybkie wygenerowanie pliku JSON zawierającego listę dostępnych dźwięków na podstawie struktury katalogów i nazw plików w określonym folderze.
ryyzka bledow 

\begin{figure}[h]
\centering
\begin{lstlisting}[caption={AnimatedCircle - fragment funkcji init}, label={code/lista}]
\end{lstlisting}
\end{figure}